...
def generate_links(query_list, links_folder, max_results=50):
    for query in query_list:
        links_file = os.path.join(links_folder, f"{query}.txt")
        print(f"Генерация ссылок для {query}, максимум: {max_results}")
        with DDGS() as ddgs:
            results = ddgs.images(f"{query} photo", region="wt-wt", size="Medium", max_results=max_results)
            unique_links = set()
...
def download_images(animals, links_folder, dataset_folder, max_images=30):
